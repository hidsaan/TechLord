...
const registerController = async (req, res) => {
    try {
        ...
        if (name && email && phone && password && confpassword !== "") {
            if (!existingUser) {
                const newUser = new User({ name, email, phone, password: hashedPassword, confpassword: confhashedpsw });
    ...
    try {
        ...
        if (email !== "" && password !== "") {
            if (findUser) {
                ...
                if (passverify) {
                    const otp = "4443"
                    const loginMail = await transporter.sendMail({
                        from: {dbEmail},
                        to: `${email}`,
                        subject: "Login OTP",
                        text: `The OTP for login is: ${otp}`
                    })

                    if (!loginMail) {
                        return res.json({ message: "Error sending OTP. Please enter correct email or try again after some time." })
                    }
                    else {
                        res.json({ message: "The OTP has been sent to you on your registered email" })
                        const enterotp = req.query
